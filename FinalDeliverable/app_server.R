...
my_server <- function(input, output) {
  output$map <- renderLeaflet({
    ...
    # Adding some parameters (such as labels,radius, latitude)
    #   for maps to display.
  ...
  output$pie <- renderPlotly({
    ...
    p <- plot_ly(room_types,
      ...
      layout(
        title = paste0(
          "Breakdown of NYC Airbnb Listings by Room Type in the ",
          input$pieNeighbourhoodInput, " Neighbourhood"
        ),
        xaxis = list(
  ...
  output$table <- renderTable({
    ...
    aggregate_table <-
      summarize(
        ...
        average_min_nights_req = round(mean(minimum_nights, na.rm = FALSE),
          digits = 1
        ),
        average_avail_nights = round(mean(availability_365, na.rm = FALSE),
          digits = 1
        )
      ) %>%
      rename(
